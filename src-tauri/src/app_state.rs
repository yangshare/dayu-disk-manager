...
mod tests {
    ...
    fn entry(stage: &str, mid: &str) -> JournalEntry {
        JournalEntry {
            ...
            dst: "D:/d".into(),
            tmp: "D:/d.tmp".into(),
            old_path: "C:/s.old".into(),
            time: "2026-07-18T00:00:00Z".into(),
            final_mark: None,
        }
    }
    ...
    fn status_serializes_snake_case() {
        ...
        assert_eq!(
            serde_json::to_string(&MigrationStatus::Active).unwrap(),
            "\"active\""
        );
        assert_eq!(
            serde_json::to_string(&MigrationStatus::TargetPendingDelete).unwrap(),
            "\"target_pending_delete\""
        );
        assert_eq!(
            serde_json::to_string(&MigrationStatus::PendingManualConfirm).unwrap(),
