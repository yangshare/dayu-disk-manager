...
import { Link2, RefreshCw, RotateCcw, Unlink, FolderOpen, LoaderCircle } from '@lucide/vue'
...
<template>
  <div class="page">
    <header class="page-header">
      ...
      <button class="button button-secondary" :disabled="store.loading || store.running" @click="store.refresh"><RefreshCw :size="15" :class="{ spinning: store.loading }" /> 刷新</button>
    </header>
    <div v-if="store.error" class="notice notice-error">{{ store.error }}</div>
    <ProgressStage :progress="store.progress" operation="restore" />
    ...
    <section class="results-panel links-panel">
      <div class="results-toolbar">
        <div class="result-count"><Link2 :size="17" /><strong>{{ store.items.length }} 条链接</strong></div>
        <div v-if="store.running" class="loading-label"><LoaderCircle class="spinning" :size="15" /> 正在还原，请勿关闭应用</div>
      </div>
          ...
          <tr v-for="l in store.items" :key="l.id">
            ...
            <td class="actions">
              <button class="button button-quiet" :disabled="l.broken || store.running" @click="store.restore(l.id)">
                ...
                {{ store.running && store.activeRestoreId === l.id ? '还原中' : '还原' }}
              </button>
              <button class="button button-secondary" :disabled="store.running" @click="onBreak(l.id)"><Unlink :size="14" /> 断开</button>
            </td>
          </tr>
        </tbody></table>
      </div>
